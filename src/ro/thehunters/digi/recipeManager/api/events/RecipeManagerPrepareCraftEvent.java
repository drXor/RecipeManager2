...
/**
 * Event triggered when RecipeManager's custom recipes' ingredients are placed
 * in the workbench and the result is displayed.<br>
 * Player can return null in certain situations, so be sure to prepare for that
 * situation.<br>
 * Event can be cancelled by setting the result to null to prevent player from
 * crafting the recipe.
 * 
 * @author Digi
 */
public class RecipeManagerPrepareCraftEvent extends Event {
	...
	public RecipeManagerPrepareCraftEvent(WorkbenchRecipe recipe,
			ItemStack result, Player player, Location workbench) {
	 ...
	 * This may also return a special item that is used to print multi-results
	 * into one item's lore!
	 * 
