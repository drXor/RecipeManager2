...
/**
 * Event triggered when RecipeManager's custom furnace recipes are beeing
 * smelted.<br>
 * Player can return null in certain situations, so be sure to prepare for that
 * situation.<br>
...
public class RecipeManagerSmeltEvent extends Event implements Cancellable {
	...
	public RecipeManagerSmeltEvent(SmeltRecipe recipe, FuelRecipe fuelRecipe,
			ItemResult result, Block block, String smelter, String fueler) {
	...
	/**
	 * Get the player's name that initially placed the ingredient for this
	 * recipe.<br>
	 ...
	 * NOTE: This returns null if player is not online or plugin couldn't get
	 * the player's name, use getSmelterName() to get his name only.<br>
	...
	/**
	 * Get the Player object of the player that placed the fuel powering this
	 * recipe.<br>
	 * NOTE: This returns null if player is not online or plugin couldn't get
	 * the player's name, use getFuelerName() to get his name only.<br>
	 * Shortcut for: Bukkit.getPlayerExact(event.getSmelterName());
	 * 
