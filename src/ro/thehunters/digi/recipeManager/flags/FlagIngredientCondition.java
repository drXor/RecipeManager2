...
public class FlagIngredientCondition extends Flag {
	...
	static {
		...
		"  failmsg <text>                   = overwrite message sent to crafter when failing to provide required ingredient.", "", "NOTE: if an ingredient exists more than once in the recipe then the conditions will apply to all of them.", "NOTE: this flag can not be used in recipe header, needs to be defined on individual results or recipes.", };
	...
	public class Conditions implements Cloneable {
		...
		/**
		 * @return a map that contains data values and if they should or not be
		 *         in the ingredient's data (the '!' char in the definition);
		 *         never null.
		 */
		...
		/**
		 * @return a map that contains data bits and if they should or not be in
		 *         the ingredient's data (the '!' char in the definition); never
		 *         null.
		 */
		...
		public boolean checkData(short data) {
			...
			if (hasDataBits()) {
				for (Entry<Short, Boolean> e : dataBits.entrySet()) {
					...
					/* Old way - makes all bitwise operations required TODO
					 * remove // If its allowed and doesn't match then return
					 * false // Or if it's not allowed and matches then also
					 * return false if(e.getValue() != ((data & d) == d)) {
					 * return false; } */
				}
		...
		/**
		 * Check the supplied item with supplied arguments against this
		 * condition class.
		 * 
		 * @param item
		 *            the ingredient, must not be null.
		 * @param a
		 *            use {@link ArgBuilder} to build arguments, must not be
		 *            null.
		 * @return
		 */
