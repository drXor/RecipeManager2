...
public class FlagPermission extends Flag {
	...
	protected boolean onParse(String value) {
		...
		for (String arg : split) {
			 ...
			 * Bukkit.getPluginManager().getPermission(arg);
			 * 
			 * if(permission == null) { permission = new Permission(arg,
			 * PermissionDefault.FALSE);
			 * Bukkit.getPluginManager().addPermission(permission);
			 * //RecipeErrorReporter.warning("Flag " + getType() +
			 * " has permission '" + arg + "' which is not registered!"); } */
	...
	/*@Override public List<String> information() { List<String> list = new
	 * ArrayList<String>(2);
	 * 
	 * String allowed = getPermissionsString(true); String unallowed =
	 * getPermissionsString(false);
	 * 
	 * if(!allowed.isEmpty()) { int i = allowed.indexOf(','); String permission
	 * = allowed.substring(0, (i > 0 ? i : allowed.length()));
	 * list.add(Messages.FLAG_PERMISSION_ALLOWED.get("{permission}", permission,
	 * "{permissions}", allowed)); }
	 * 
	 * if(!unallowed.isEmpty()) { int i = unallowed.indexOf(','); String
	 * permission = unallowed.substring(0, (i > 0 ? i : unallowed.length()));
	 * list.add(Messages.FLAG_PERMISSION_UNALLOWED.get("{permission}",
	 * permission, "{permissions}", unallowed)); }
	 * 
	 * return list; } */
}
