...
public class MultiResultRecipe extends BaseRecipe {
	 ...
	 */
	/* TODO remove ? public List<ItemResult> getResults(Args a) {
	 * List<ItemResult> list = new ArrayList<ItemResult>(results.size());
	 * 
	 * for(ItemResult r : results) { r = r.clone(); a =
	 * ArgBuilder.create(a).result(r).build();
	 * 
	...
	/**
	 * @return true if recipe has more than 1 result or has failure chance (2
	 *         results, one being air), otherwise false.
	 */
	...
	public ItemResult getFirstResult() {
		...
		// TODO remove ?
		/*for(ItemResult r : results) { if(r.getType() != 0 &&
		 * !r.hasFlag(FlagType.SECRET)) { return r; } }
		 * 
		 * // if no non-secret result was found, then we must return
		 * something... */
		for (ItemResult r : results) {
			if (r.getType() != Material.AIR) {
				return r.clone();
	 ...
	 * 
	 * @param a
	 *            dynamic arguments, use {@link ArgBuilder#create()} to build
	 *            arguments for this.
